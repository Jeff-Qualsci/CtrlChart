...
Control charts for continuous data use the within run variability ($\sigma_{wi}$) to estimate the common cause variability for the population and calculate the control limits. The primary data chart used is the $\overline{X}$ Chart, which shows the run means ($\overline{X}$) and reference lines for the population mean (CL) and the upper and lower control limits (UCL, LCL) which are calculated as shown:

$$CL = mean(\overline{X})$$ {#eq-cl}

$$Control Limits = CL \pm 3(\sigma_{wi})$$ {#eq-lims}
...
Statistical analysis of the data points from both of these charts are used to signal that the process may be out of control. The most basic test compares the summarized statistic for each run (mean for the data chart and standard deviation or range for the variability chart) with the control limits for that statistic that are calculated from all the available runs. If the data point falls outside of these limits, it triggers an "Outside Limits" flag. This is discussed in more detail in @sec-cRepsOutlier.

The second type of test looks for trends in the data points which differ from what would be expected, if the variatbility between runs were random. While there a number of different statistical tests which fit in this category, the details of the specific methods used in the webtool are presented in @sec-meanShift. These trend tests can identify changes in the process before the data points move outside of the control limits and provide an early warning to potential problems in the process before the measurement values go outside of acceptable limits.

It is important to remember that these flags are just indication that the assay may be out of control. The assay should be investigated to determine the primary cause(s) for the discrepancy. No changes should be made to the protocol unless there is an obvious observable cause (e.g a liquid handling issue that was observed during the experiment). If the error is just extreme random noise, then the data should regress toward the mean without any intervention. If a root cause is identified and changes are made to the protocol, the new protocol may need to be revalidated with a larger set of reference compounds using the [Replicate-Experiment](https://agm.ncats.nih.gov/app/) [@beck2004], to ensure that there are no systematic shifts in the data.

## Reference compound(s)

A reference compound refers to a molecule which is tested in every experimental run of an assay. This compound may be tested once per run, on every plate in a run, or on multiple plates in a run, depending on the characteristics of the assay and its sample capacity. A reference compound should be prepared and tested in the assay as if it were any of the test compounds. It should exhibit a full sigmoidal dose response curve (well defined top, bottom, and AC~50~) within the dynamic range of the assay, for the most reproducible potency determinations. If a single reference compound is used, it is also recommended that its potency is near the lower limit of the current potency range, since the potency of test compounds tends to improve over time. Finally, there should be an ample supply of the reference compound so that it can be used over a large number of runs.
...
Control charting the potency of reference compound(s) is a simple way to document that an assay is providing consistent data over an extended period of time. Any potency measurement can be used (e.g. AC~50~, K~i~, ED~80~, ...). However it is important to remember that potency values show a log-normal distribution [@elassaiss-schaap2020]. Therefore, as with the replicate-experiment [@beck2004], the data must be transformed to their log values for the statistical analysis. The statistics can then be transformed back to the linear scale for graphing and will appear to be normally distributed, if the y-axis is log scaled. It should be noted that the transformation of standard deviations determined on the log transformed data become fold-standard deviations when transformed back to the linear scale.

Since most experiments do not usually have enough replicates within the run to provide a good estimate of the population variability, adjustments must be made for the sample size within each run [@guthrie]. These adjustments use a factor, c~4~, whose value is determined by the number of replicates within each run as shown: $$\sigma_{wi} = \overline{sd_{wi}}\sqrt{1 - c^2_4}$$ {#eq-sdwi}
...
1.  If all of the runs contain 2 or more replicates, generate a Replicates report (where the within run variability is based on the measured within run standard deviations).
2.  If most of the runs are single replicates, then generate an Individuals report (where the within run variability is based on the moving range between runs) using the first replicate in every run.
...
The first example may be less common in practice, but is the simplest to interpret, because all of the runs contain the same number of replicates (n = 6), so the c~4~ factor (@eq-sdwi) is the same for each run and the control limits are simple horizontal lines representing the 99% Confidence Interval.
...
![MSR chart of the constant replicate data.](Reports/cRepsData/cRepsData_MSRFig.png){#fig-cRepsMSRChart}
...
cRepsMSRHead <- read_csv("Reports/cRepsData/cRepsData_MSRData_tbl.csv") %>% 
  slice_head(n = 10) %>% 
  gt() %>% 
...
$$sd_\mathit{wi} = \frac{\bar{R}}{d_2}$$ {#eq-rsd}
...
While the value of d~2~ is dependent upon the size of the data set, for n = 2, the value of d~2~ is 1.128. This can then be used to estimate sd~wi~ and provides the information necessary to generate data and variability control charts as shown in the following example.
...
![Moving Range chart of the single replicate data.](Reports/sRepData/sRepData_MRaFig.png){#fig-sRepMRChart}
...
sRepMRchartStatsHead <- read_csv("Reports/sRepData/sRepData_MChartData_tbl.csv") %>% 
  mutate(n = as.character(n)) %>%
  slice_head(n = 10) %>% 
  gt()  %>% 
  fmt_number(n_sigfig = 3) %>% 
...
![MSR chart of the single replicate data,](Reports/sRepData/sRepData_MSRFig.png){#fig-sRepMSRChart}
...
sRepMSRHead <- read_csv("Reports/sRepData/sRepData_MSRData_tbl.csv") %>% 
  slice_head(n = 10) %>% 
  gt()  %>% 
  fmt_number(n_sigfig = 3) %>% 
...
If a reference compound is included on every plate tested, it's likely that the number of replicates will vary from run to run. While this might be a common situation, this will add some complexity to the control charts. Most runs (outside of HTS) will be small (\< 20 plates), so statistical factors are used to adjust for sampling bias in order to better estimate the population $\sigma_{\mathit(wi)}$. As described in the previous 2 sections, there are different factors for this estimate, depending on whether the range (d~2~, @eq-rsd ) or the sd (c~4~, @eq-sdwi ) is being used. @tbl-cChartFact shows how these factors change with different values for n. The values for these factors of these factors increase asymptotically as n increases and will eventually converge on constants that are used for larger sample sizes.
...
When the number of replicates is variable, the webtool will use all of the data from all of the runs with the same number of replicates to calculate the upper and lower control limits for that group of runs. The following data set illustrates the last situation with 1-6 replicates/run.
...
![S chart of the variable replicate data.](Reports/vRepsData/vRepsData_SchFig.png){#fig-vRepsSChart}
...
vRepsSchartStatsHead <- read_csv("Reports/vRepsData/vRepsData_SChartData_tbl.csv") %>% 
  mutate(n = as.character(n)) %>%
  slice_head(n = 10) %>% 
  gt() %>% 
  fmt_number(n_sigfig = 3) %>% 
...
While they only use the data from the first replicate of each run, Individuals and Moving Range charts are produced whenever there is at least one run with a single replicate of the reference compound. This may help to fill the gaps that appear in the Xbar and S charts. Additionally, whenever the majority of the runs have a single replicate of the reference compound, the Individual and Moving Range charts will be the only control charts generated by the webtool.

![Individuals chart of the variable replicate data.](Reports/vRepsData/vRepsData_IChFig.png){#fig-vRepIChart}
...
vRepIndStatsHead <- read_csv("Reports/vRepsData/vRepsData_IChartData_tbl.csv") %>% 
  slice_head(n = 10) %>% 
  mutate(n = as.character(n)) %>%
  gt() %>% 
  fmt_number(n_sigfig = 3) %>% 
...
![Moving Range chart of the variable replicates data.](Reports/vRepsData/vRepsData_MRaFig.png){#fig-vRepMRChart}
...
vRepMRchartStatsHead <- read_csv("Reports/vRepsData/vRepsData_MChartData_tbl.csv") %>%
  mutate(n = as.character(n)) %>%
  slice_head(n = 10) %>% 
  gt() %>% 
  fmt_number(n_sigfig = 3) %>%  
...
![MSR chart of the variable replicates data.](Reports/vRepsData/vRepsData_MSRFig.png){#fig-vRepsMSRChart}
...
vRepsMSRHead <- read_csv("Reports/vRepsData/vRepsData_MSRData_tbl.csv") %>%
  slice_head(n = 10) %>% 
  gt()  %>% 
  fmt_number(n_sigfig = 3) %>% 
...
### Establishing Control Limits {#sec-ctrlLims}
...
extract_ctrlCht_params <- function(endDate, ccData, ccType) {
  ...
  params <- qic(x = ccData$Run, y = log10(ccData$Data), chart = ccType, return.data = TRUE) %>%
    ...
    mutate(across(contains("cl"), \(x) x^10)) %>%
    summarize_all(last)
...
The variability in the parameters for the mean potency is shown in @fig-xBarTL, for each data set with the center line (CL) in blue and the control limits (UCL, LCL) in red. While all of the data sets show some significant differences in the control limits in the first few runs, they tend to converge to a steady state after about 6 runs (Note: 6 runs is also the lower limit for MSR estimation). The differences between the y-axes for the different runs also illustrates the impact of increasing n on the parameter estimates.
...
### Outside Limits Flag {#sec-cRepsOutlier}

Ideally, a new control chart should be created and reviewed as soon as possible after each run. If the data for the reference compound(s) appear to be in control (no flags in the Mean Shift or Outside Limits columns), then the experiment can be considered to be successful.

The Outside Limits flag is simply a test of the of the summary statistics (mean and sd~wi~) for each run within the entire data set. These statistics are compared to their control limits (@eq-cl), which correspond to a 99% CI for the run statistic. If the run statistic falls outside of these limits, it is flagged as being out of control.
...
![](Reports/cRepsOutData/cRepsOutData_XbrFig.png)

![](Reports/cRepsOutData/cRepsOutData_SchFig.png)
...
### Mean Shift Flag {#sec-meanShift}

The mean shift flag is used to identify suspicious trends in the data related to a group of consecutive runs. This flag is based on the assumption that the within run variability is random, therefore half of the run statistics should be on either side of the population statistic (Center Line). Conceptually this can be treated like coin flips. Overall you expect half of the results will be heads or tails, but short streaks of heads are not suspicious the the more times you flip the coin the longer these streaks need to be for them to cause concern. This can be tested statistically by looking at how many times the line connecting the run summarized statistics cross the Center Line (population statistic). This flag actually encompasses 2 measured parameters. The first measurement is the observed number of times that the data line crosses the center line and the second measurement is the number of consecutive runs after the last crossing of the center line. These measurements can be compared to test statistics based on the number of runs in the data set to identify suspicious behavior [@anhøj2014].
