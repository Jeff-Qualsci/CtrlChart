...
The data and statistics are shown in . Since this method uses only the first replicate in every run, there are no gaps in the graph or NA values in the data table.
...
#| fig-cap: MSR chart of the variable replicates data.
...
```{r}
#| label: fig-cRepsOutxBar
#| fig-cap: Xbar control chart for suspect run

# cRepsData up to suspect run
cRepsOutData <- filter(cRepsData, Run <= runDates[20])

cRepsOutReport <- xbars_charts(cRepsOutData, usrtitle = "Constant Replicates")

cRepsOutCharts <- cRepsOutReport$XbarChart / cRepsOutReport$SChart

cRepsOutCharts
```

@fig-cRepsOutxBar shows the control charts for the suspect run. Since the last data point in the Xbar chart falls just below the Lower Control Limit, the run needs to be investigated. However, the S chart shows that the variability for that run appears to fine, so it is possible that this is simply random noise.

```{r}
#| label: tbl-cRepsOutRun
#| tbl-cap: Replicate data for the suspect run.

cRepsOutRun <- cRepsOutData %>% 
